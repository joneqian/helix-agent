...
# ---------------------------------------------------------------------------
# _poll_disconnect unit tests — drive the loop with a stub.
# ---------------------------------------------------------------------------


class _FakeDisconnectSource:
    """Mutable disconnect flag returned by ``__call__``."""
...
async def test_poll_cancels_token_when_disconnect_observed() -> None:
    token = CancelToken()
    source = _FakeDisconnectSource()
    poll_task = asyncio.create_task(_poll_disconnect(source, token, poll_interval_s=0.005))
...
async def test_poll_exits_quickly_when_token_already_cancelled() -> None:
    ...
    await asyncio.wait_for(
        _poll_disconnect(_FakeDisconnectSource(), token, poll_interval_s=10.0),
        timeout=0.1,
    )
...
async def test_poll_treats_receive_exception_as_disconnect() -> None:
    ...
    token = CancelToken()
    await asyncio.wait_for(
        _poll_disconnect(_Raising(), token, poll_interval_s=0.001),
        timeout=0.5,
    )
...
async def test_poll_fires_on_cancel_callback() -> None:
    ...
    await asyncio.wait_for(
        _poll_disconnect(source, token, poll_interval_s=0.001, on_cancel=_cb),
        timeout=0.5,
    )
