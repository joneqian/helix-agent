"""Unit tests for the J.8 HITL eval — Stream J.13a closeout."""
...
from hitl import (  # type: ignore[import-not-found]  # noqa: E402
    HitlCase,
    evaluate_set,
    load_cases,
)
...
def test_load_cases_parses_seventeen() -> None:
    cases = load_cases(_EVAL_DIR / "datasets" / "hitl" / "m0_baseline.yaml")
    assert len(cases) == 17
...
async def test_unknown_scenario_marks_case_failed() -> None:
    ...
    case = HitlCase(case_id="bogus", scenario="not_a_scenario")
