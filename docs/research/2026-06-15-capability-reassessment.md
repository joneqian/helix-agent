...
| | 旧 | 新 | T0+T1 | +T2 | +8.5 | +13.2 | +14.4 | +9.4 | +16.3 | +10.5 |
|---|---|---|---|---|---|---|---|---|---|---|
| 总分 | 388/430 | **396/430** | **402/430** | **404/430** | **406/430** | **408/430** | **410/430** | **413/430** | **415/430** | **417/430** |
| 百分 | 90.2% | **92.1%** | **93.5%** | **93.95%** | **94.4%** | **94.9%** | **95.3%** | **96.0%** | **96.5%** | **97.0%** |
| 均分 | 4.51 | **4.60** | **4.67** | **4.70** | **4.72** | **4.74** | **4.77** | **4.80** | **4.83** | **4.85** |
...
>
> **9.4 跨进程恢复 ★2→★5（#677+#678）**：Postgres 租约（`claimed_by`/`lease_until`/`heartbeat_at`）+ 孤儿
> sweep（reclaim CAS 恰一赢家）+ **自动热接力**——peer 实例 `adopt` 重认领的 run，`run_agent(graph_input=None)`
> 从 durable LangGraph checkpoint 续跑（13.2 幂等兜底）。**live 实证（蓝绿双实例真栈 kill blue mid-run）**：
> green sweep 检测过期租约→reclaim→续跑到 `status=success`，owner 易主 / `reclaim_count=1` / `run:failover`
> 审计 / metric 0→1，5 项全绿。live 暴露真依赖：failover resume 硬要 `checkpointer_backend=postgres`（dev 默认
> memory 进程内 saver→`EmptyInputError`，确定性单测 monkeypatch run_agent 漏判）+ 续跑 event seq 撞键（已修
> `RunEventStore.next_seq`）。**域9 持久化 4.00→4.50**（9.4 +3）。
>
> **运维韧性三连（#679/#680/#681）**：16.3 应用层 backpressure ★3→★5——`BackpressureMiddleware`
> 过载守卫，in-flight 深度（复用 `Lifecycle.in_flight`）超阈值 503+Retry-After 甩负，置 Observability 内/Auth 外，
> 默认开 cap 512（**域16 4.20→4.60**）。10.5 SLO 烧录率 ★3→★5——`burn_rate.yml` Google SRE 多窗多烧录率
> （record `helix_slo_burn_rate{slo,window}` + 长窗确认/短窗加速告警 P0/P1/P2），promtool check+test 双 SUCCESS，
> 关掉 sli.yml「Burn-rate rules are M1」deferral（**域10 4.50→4.83**）。15.3 维持 ★5——`sanitize_dangling_tool_calls`
> 此前无直接单测，补纯函数边缘 9 例（含跨递归多 AIMessage 位置）+ wrapper 3 例；subagent cancel 传播已由
> `test_subagent` 覆盖（「跨递归 cancel 薄」部分过期）。共 +4★，415→**417/430**。
...
**域13 4.60→5.00**(13.2 ★3→★5 幂等,#673) · **域14 4.60→5.00**(14.4 ★3→★5 MCP 审计,#675) ·
**域9 持久化 4.00→4.50→5.00**(9.4 ★2→★5 failover #677+#678,再 9.5 ★2→★5 分布式队列,均 live 实证 #683+#684+#685) ·
**域16 部署运维可靠性 4.20→4.60**(16.3 ★3→★5 应用层 backpressure 过载守卫) ·
**域10 可观测性 4.50→4.83**(10.5 ★3→★5 SLO 多窗烧录率落 infra/)。其余 6 域不变。
（15.3 维持 ★5——resume 消毒测试债清,不改分。）
...
- **未变的 design-only 风险项**：12.4(计费业务层)/16.4(基础设施级自愈,k8s 层)——均诚实标
  ≤★3 且有明确 M1/业务层归属,非静默缺失。（9.4 failover + 9.5 分布式队列均已 ★2→★5 live 收口 #677+#678 /
  #683+#684+#685;16.3 应用层 backpressure 已 ★3→★5;3.3/13.2/14.4 亦已先后清。）
...
- M1 真大头：9.5 分布式队列、12.4 计费业务层、16.4 基础设施级自愈(k8s)、7.6 IDS、7.2 isolation_level(dedicated_node 那档 M1)（9.4 failover/14.4 MCP 审计/16.3 backpressure 已清）
