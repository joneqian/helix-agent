...
| **4.1 工作记忆** | ★3→★4 | 低估 | CM-2 廉价滑窗 `context/working_window.py:WorkingWindow` 已接线（`agent_factory.py:559`）+ 测（`test_working_window.py` 20+）；旧标"缺廉价滑窗"已补 | ✅ 亲核接线 |
...
**域级变动**：域2 5.00→4.86(2.2↓) · 域4 4.57→4.71(4.1↑) · **域7 3.63→4.25**(7.2/3/4/6 ↑5,PI+HX-10 主升) ·
