...
- `src/api/eval-runs.ts` SDK（对 `/v1/eval-runs` POST/GET，envelope 对账）
- `EvalRunsList.tsx`（runs 列表 + 触发按钮）+ `EvalRunDetail`（状态/summary/per-case + 11.3 会话级指标 + 11.4 trace 断言 + 11.5 对抗结果）
- SE-8 接线点全套：router/Sidebar/CommandPalette/i18n 双语/Storybook/Playwright/TenantScope
- 先合 admin-ui 设计基线再写 React。1 页覆盖 11.3/11.4/11.5/11.6 的可视化。
