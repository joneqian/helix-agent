...
## 7. 首次安装向导(admin-ui 内建首个 system_admin)— Mini-ADR ACCT-2

运维不碰 Keycloak 控制台,在 admin-ui 内造出第一个平台 system_admin。补「白标」最后一块:连建账号都不露 Keycloak。

### 决策(用户拍板)
- 向导建的是**平台 system_admin**(非租户 admin);租户 admin 之后用邀请流。
- system_admin 归属一个**专用「平台/系统」租户**(`settings.platform_tenant_id` 保留 UUID `1111…`,display "Platform"),业务租户后续单独建。
- 密码**向导里直接输**(emailVerified=true + 非临时密码 → 立即可登录,不依赖 SMTP)。
- `SETUP_TOKEN` **始终强制**(任何环境):无 token 配置则 `/v1/setup` 直接拒。

### 后端
- `POST /v1/setup`(**豁免认证**,加入 `auth_exempt_path_prefixes`)+ `GET /v1/setup/status`(返 `{initialized, setup_enabled}`)。
- 双门控:① `X-Setup-Token` 常量时间比对 `settings.setup_token`(未配=403 SETUP_NOT_CONFIGURED);② 零-admin 不变量(`list_platform_scope` 非空=409 ALREADY_INITIALIZED,天然一次性)。
- `provision_platform_admin`:建平台租户(bypass_rls,幂等 get-then-create)→ KC `create_user(email_verified=True)` → `reset_password(temporary=False)` → 复用 `bootstrap_system_admin` 授绑定 → 审计(TENANT_CREATE + ROLE_BINDING_CREATE)。
- KC `create_user` 加 `email_verified: bool=False` 参数(Protocol+Http+Fake;默认 false 不动邀请流)。

### 安全
- 无认证端点最大风险=部署窗口劫持(谁先 setup 谁成 admin)→ **SETUP_TOKEN 是核心防线**(运维专属,部署设 env,首跑后清除)。零-admin 门控是第二道(防重复)。
- KC 失败留下的只有幂等平台租户行;重试不重复建。

### 前端
- `/setup` 路由(ProtectedRoute 外);启动查 `/v1/setup/status` 未初始化 → 导向向导。
- 向导表单:平台名(默认 Platform)+ admin 邮箱 + 密码 + 显示名 + setup token → 提交 → 成功 → 跳登录。
