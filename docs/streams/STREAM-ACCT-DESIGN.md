...
## 6. 满分判定(零技术债)
- [ ] 无 TODO 残留
- [ ] 单测覆盖五条件矩阵 + 跨租户 SET ROLE 真 PG 验
- [ ] 文档同步(本档 + runbook bootstrap-admin 补 env 路径 + ITERATION-PLAN)
- [ ] 可观测:自动升格记审计(沿用 role_binding create 审计)
- [ ] CI 全绿(ruff/mypy/pytest/vitest)
- [ ] 无 bug 遗留
