...
| 2026-06-13 | v2.7 | **H.9 收尾 + H.6–H.9 前端债全清**:PR1 全交付——SDK `rate_card.ts`(5 方法全 envelope + µ$→$/1M 换算)+ SettingsRateCard(H-22 门控不发 fetch / 列表过滤+include_expired / create modal 预校验+换算只读提示 H-21 / 编辑抽屉只开放 Patch 可变字段+身份只读+重定价新建引导 H-20 / delete)+ 全接线 + Storybook + vitest 5(含 patch payload 不含身份字段断言)+ Playwright 门控冒烟。设计 § 6.10 全兑现无偏差。**前端债四项(H.6/H.7/H.8/H.9)全部收尾**(#581–#588 + 4 实现 PR);遗留 follow-up:H-10 SQL JOIN(M2)/ H.7-F1 / H.8-F1(均后端能力,另立项) |
