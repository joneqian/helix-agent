...
| 迁移 | 表 | 子项 | RLS |
|------|-----|------|-----|
| `0015_user_scope` | `app_user` + 给 `thread_meta` 等加 `user_id` 列 | J.14 | tenant RLS + user 列 |
...
- **J.14 是硬前置**：J.3 / J.15 / J.9 的表都带 `user_id`,必须先有 `app_user` + scope 约定。J.14 与 Wave 1 可并行（Wave 1 不碰 per-user 数据）。
...
helix 隔离做到 **per-租户**：Postgres RLS（GUC `app.tenant_id`）+ `build_rls_sessionmaker` 每事务 `set_config`。`user_id` 不是一等隔离 scope —— thread / session 归属靠授权层临时判断,无持久 `app_user` 概念。per-user 持久 agent 形态要求"用户"成为一等实体。
...
| `user_id` 来源 | OIDC `sub` claim（prod）/ dev header（dev）→ control-plane 在 run 入口解析 → 落 `app_user` 表 + 写入 RLS context |
...
```python
# 迁移 0015_user_scope
class AppUserRow(Base):                    # 表 app_user
    id: UUID                               # 内部 user_id
    tenant_id: UUID                        # RLS scope
    external_subject: str                  # OIDC sub / dev 标识
    display_name: str
    created_at, last_active_at: datetime
    # UNIQUE(tenant_id, external_subject)

# thread_meta / sandbox_instance / 等加列：user_id: UUID (FK app_user)
```

- `helix-persistence/rls.py`：加 `RLS_USER_GUC_NAME = "app.user_id"` + `current_user_id_var: ContextVar`;`build_rls_sessionmaker` 的 `after_begin` 监听器同时 `set_config` 两个 GUC。
- control-plane run 入口中间件：解析 `user_id` → upsert `app_user` → 设两个 ContextVar。
...
背景：per-user 持久 agent 要求"用户"成为一等实体。备选：(a) 给所有表加用户级 RLS;(b) user_id 一等列 + 应用层授权,仅 per-user 数据表加防御性用户级 RLS。取舍：选 (b) —— 同租户用户 = 同公司同信任域,硬隔离边界本就是租户;全表用户级 RLS 增加每查询开销且语义过严（同租户审计 / 客服需跨用户读）。per-user **数据表**（记忆 / 工作区 / 产物）泄漏后果重,加用户级 RLS 做纵深。
