...
- **CM-4（B5）reranker 接通** P1：memory recall Hybrid 后接 cross-encoder rerank（配置已预留 migration 0051）
  - [ ] **CM-4 PR1 记忆召回接 rerank（orchestrator）**：`make_memory_recall_node` 加可选 `reranker`（从 tools/knowledge 导 `Reranker`）+ 宽召回 `max(top_k,20)` + `_rerank_memories`（best-effort 回落 `candidates[:top_k]`）；`MemoryEnv.reranker` 字段；factory 透传；`record_memory_rerank{outcome}` + unit/integration（fake reranker 重排/宽召回 limit/降级不丢记忆/None 零变更）
  - [ ] **CM-4 PR2 control-plane 激活**：`app.py` `MemoryEnv(..., reranker=reranker)`（复用已建 DynamicResolvingReranker）+ 测试断言 MemoryEnv 携带 reranker + 文档标 done
