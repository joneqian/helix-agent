...
- **CM-0 实现**（地基，先行）：
  ...
  - **CM-0 PR2b-ii ingest 接线**：`SandboxWorkspaceReader`（`tools/file_ops.py`，包 `build_read_wrapper`/`run_in_sandbox`）+ entry-chain `workspace_ingest_node`（planner 后 agent 前，每 ainvoke 一次=run 始/resume）+ 注入扫描校验 + `STATE_INGESTED` + 漂移裁决（DB 权威）+ agent_factory gate + integration（人改 PLAN.md→resume→回灌）
