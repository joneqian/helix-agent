...
- [~] **J.8 人在回路 / 审批** — LangGraph `interrupt()` 审批节点 + control-plane resume 端点 + `PolicySpec` 声明式门控 + agent 主动 `ask_for_approval` 工具。**2026-05-20 修订（Mini-ADR J-24）+ 2026-05-22 设计修订**：M0 必含**审批超时 fallback**（manifest 可配 / 默认 24h 自动 reject + audit）+ audit trail（APPROVAL_REQUESTED / APPROVAL_DECIDED）+ Admin UI H.3 审批面板接入 + **agent 主动请求路径**（deer-flow 对比启发，与声明式门控并存）+ **reason_kind 5 类型分类**。STREAM-J-DESIGN § 14（重写 + 加 § 14.5~14.9）+ Mini-ADR J-15 / J-24（含 (4)(5)）。**~5-10% 已实装**（仅 audit_log 表 + PolicySpec 容器 + AgentState 框架）。**2026-05-22 进行中（5 PR 链）**：
  - [~] **设计 PR** — STREAM-J-DESIGN § 14 重写 + § 14.5 ask_for_approval / 14.6 reason_kind / 14.7 超时 / 14.8 audit / 14.9 边界 + Mini-ADR J-24 修订加 (4)(5)
  - [ ] **J.8-step1**: protocol approval.py（ApprovalRequest/Decision/ReasonKind）+ AgentState.pending_approval + PolicySpec 审批字段 + AuditAction 两态
  - [ ] **J.8-step2**: approval 图节点 + interrupt() + ask_for_approval builtin tool + middleware 拦截
  - [ ] **J.8-step3**: control-plane resume API 3 态 + GET /runs/{id} 含 pending_approval + audit emit + 24h 超时 job
  - [ ] **J.8-step4**: eval module（tools/eval/hitl.py）+ baseline regen + ITERATION-PLAN [x] + 零债 6 条
