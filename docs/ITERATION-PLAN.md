...
- **CM-1（A1）运行时 error-as-guidance** P0：通用工具失败→grounded 恢复 advisory 注入主循环（泛化 L-4 `<mutation-advisory>`；注入须源自真实工具/执行信号）
  - [ ] **CM-1 PR1 分类器纯核心**：`graph_builder/error_classifier.py`（`ToolErrorClass` taxonomy + `ClassifiedToolError` + `classify_tool_error` 纯函数 + `render_recovery_advisory` + retry 受 spec 约束）+ unit。不接图、不动 state、不碰 L-4（pure-core 先行）
