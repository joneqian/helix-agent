...
- **CM-3（A3）压缩前 flush** P1：compressor 压缩前回调 → 复用 `memory_writeback` 中途落盘
  - [ ] **CM-3 PR1 共享抽取核心 + compressor 回调**：`memory.py` 抽 `flush_messages_to_memory`（行为保持地从 `memory_writeback_node` 提取，节点变薄）；`compressor.py` `compress`/`_compress_once` 增 `on_pre_compaction`（丢弃中段前 await）+ unit（抽取核心各路径/writeback 重构不回归/回调时序+入参+失败不阻断+None 等价）。不接 factory、不动 policy
  - [ ] **CM-3 PR2 接线**：`ContextCompressionPolicy.flush_before_compaction`；factory memory 启用时构造 `pre_compaction_flush` 回调传 `build_react_graph`；agent_node 调 compress 绑 config+token；`helix_cm_precompaction_flush_total{outcome}`+`helix_cm_precompaction_flush_memories`；集成测（compaction 触发 flush 到 fake store+memory 未启用零 flush+开关关闭）+ 文档标 done
