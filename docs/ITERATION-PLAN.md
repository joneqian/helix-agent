...
- [~] **J.9 产物 / Artifact 管理** — artifact + artifact_version 两表 + tenant+user 组合 RLS + 内容经 supervisor 代理读取 + 惰性回填 size/sha256。**2026-05-20 修订（Mini-ADR J-25）+ 2026-05-21 设计修订**：M0 加 lifecycle（保留期 90 天 / DELETE/PATCH/versions API / 卷满清理策略）+ quota 接入 Stream C.5 + audit trail 三态 + **MIME-aware download + XSS 防护**（HTML/SVG 强制 attachment，deer-flow 对比启发 — (c) 红线）；病毒扫描显式 (a) 推 M2。STREAM-J-DESIGN § 10（重写 + 加 § 10.5~10.11）+ Mini-ADR J-11 / J-25（含 (4) MIME/XSS）。**~50% 已实装 main**（protocol/migration/Store/tools/list+download/3 套测试）。**2026-05-21 进行中**：
  - [~] **设计 PR** — STREAM-J-DESIGN § 10 重写 + § 10.5 MIME/XSS / § 10.6 lifecycle / § 10.7 quota / § 10.8 audit / § 10.9 endpoints / § 10.10 eval / § 10.11 边界 + Mini-ADR J-25 修订加 (4) MIME/XSS
  - [ ] **J.9-step1**: 迁移 + lifecycle 状态机（soft-delete/restore/archive 三态）+ retention cron 扩 artifact 维度（复用 K3 模式）
  - [ ] **J.9-step2**: Quota 接入 C.5（artifact_download_count_30d + artifact_storage_bytes）+ 端点准入 + 429
  - [ ] **J.9-step3**: DELETE / PATCH / versions API + Audit trail 三态 + MIME-aware download + XSS 防护
  - [ ] **J.9-step4**: eval module（tools/eval/artifact.py 14 case）+ baseline regen + ITERATION-PLAN [x] + 零债 6 条
