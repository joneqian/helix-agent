...
- [ ] **J.13a 逐能力 eval 场景集（M0）** — 7 已交付能力（J.1 / J.2 / J.3 / J.6 / J.11 / J.14 / J.15）各 1 个 eval module + dataset；8 deferred 能力（J.4 / J.5 / J.7 / J.8 / J.9 / J.10 / J.12 / J.13）写 skeleton stub；aggregator `tools/eval/run_baseline.py` + checked-in baseline `tools/eval/baselines/m0_gate_baseline.yaml` 作为 Stream M Gate 锚点。**2026-05-20 修订（Mini-ADR J-28）**：J.13 拆 3 子项，**M0 仅交付 J.13a**；~~J.13b 在线采样 + LLM-judge 配额 + budget cap~~ → M1 早期 / 并入 M2-D；~~J.13c CI 回归门 + flakiness 缓解~~ → M1 早期 / 并入 M2-D。**2026-05-21 设计 PR（Mini-ADR J-37 / J-38 / J-39）**：(a) per-cap metric 类型矩阵（deterministic 优先，LLM-judge 限用 J.1）；(b) baseline = checked-in YAML + 能力 PR 同步更新（不另起 PG 表，`eval_dataset` 仅 J.12 策划用）；(c) LLM-judge = Haiku 4.5 + `temperature=0.0` + N=3 重跑；详见 STREAM-J-DESIGN § 18.3-§ 18.6。STREAM-J-DESIGN § 18 + Mini-ADR J-20 / J-28 / J-37 / J-38 / J-39
