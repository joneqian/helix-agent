...
- [~] **J.4 Sub-agent / 多智能体委派** — agent-as-tool + 取消链穿透 + 构建期深度上限 3 × max_iterations + **真正的并行 fan-out**（J.4-补强-2 取消 M2-B 推迟，M0 内交付）。STREAM-J-DESIGN § 11 + Mini-ADR J-12 / J-21 / **J-40 (2026-05-21 新增)**。**已完成 5 PR**：
  ...
  - **进行中 — J.4-补强-2 拆 4 PR（2026-05-21 设计 PR 完成；用户阅读 DeerFlow 2.0 文章后决策取消 M2-B 推迟）**：
  - [~] **设计 PR (本 PR)** — STREAM-J-DESIGN § 11 重写 + Mini-ADR J-40（M0 内交付并行 fan-out + cycle detection + global deadline + fan-in 聚合 + `AgentState` 扩展）
  - [ ] **PR 2 基建** — `SubagentStatus` 6 态（PENDING/RUNNING/COMPLETED/FAILED/CANCELLED/TIMED_OUT）+ `SubAgentInvocation` dataclass（helix-protocol）+ `AgentState.subagent_invocations` 通道（`Annotated[list, add]` reducer，参照 `reflections`）+ `TOOL_ALLOWED_STATE_KEYS` 加键 + `SubAgentTool` 3 outcome 各 emit invocation 经 `ToolResult.state_updates`
  - [ ] **PR 3 并发 fan-out** — `ToolSpec.is_parallel_safe: bool = False` + `SubAgentTool.spec.is_parallel_safe = True` + `plan_stages` 同 stage 收集并发 + `agent_factory._detect_subagent_cycle` 构建期 DFS + `ToolContext.deadline_at` + `sse.run_agent` 经 config 传播 deadline + 集成测试覆盖并发 / partial fail / cycle / deadline 4 类
  - [ ] **PR 4 eval + 收尾** — `tools/eval/sub_agent.py` 扩 `parallel_fanout` + `cycle_detection` scenarios（+4 case） + baseline regen + ITERATION-PLAN J.4 行 `[~]` → `[x]` + 零债 6 条核验
  - **(a) 显式推 M1+**：~~控制平面 wiring TrajectoryRecorder 接 runs.py run_agent 调用~~（L7 production 路径未拼，J.4-补强 PR 把 hook 接好；待 L7 production wire-up 一并完成）/ ~~子 SSE 进度流回父~~（M2-B 后续；当前 fan-out 子 result 只在 ToolResult.content + meta，没流式子进度）
...
- [x] **J.6 多模态输入** — 图像输入能力解析式双路:`ModelSpec.supports_vision: true` 走 content block(主模型直看像素), `supports_vision: false` 走 `ask_image` 工具路由到 `vision:` 块单独声明的 VL 模型;统一经 `POST /v1/sessions/{thread_id}/uploads` 对象存储摄取(`helix://image/...` ref + base64 不进 checkpointer);构建期按 `ModelSpec.supports_vision` 决议、能力不匹配则 422 / `AgentFactoryError`。PR #167–#171 全合(对象存储 wiring → LLM adapter content blocks → upload 端点 → Path A → Path B)。STREAM-J-DESIGN § 13 / Mini-ADR J-14。
  ...
  - [ ] **J.6.补强-1** Upload quota 接入 Stream C.5 QuotaService（`image_upload_count_30d` + `image_storage_bytes` 两类计费 + 429 超额）— Mini-ADR J-30
  - [ ] **J.6.补强-2** Upload 单独 audit trail（新 `AuditAction.IMAGE_UPLOAD` 含 file_size / mime / object_key / sha256 / 调用方主体）— Mini-ADR J-31
  - [ ] **J.6.补强-3** Image lifecycle（`image_upload` 表 + soft-delete + `DELETE /v1/uploads/{id}` + 保留期默认 90 天 + retention-cleanup 加图维度 + session/user 销毁级联）— Mini-ADR J-32
  - [ ] **J.6.补强-4** Path B VL 模型 fallback chain（`VisionSpec.fallbacks: list[ModelSpec]` + `AskImageTool` 复用 `LLMRouter._call_one` 错误归类）+ EXIF strip（Pillow 剥离）+ multi-image 集成测试 — Mini-ADR J-33 / J-34
