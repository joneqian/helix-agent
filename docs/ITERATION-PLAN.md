...
- [ ] **HX-9 租户级出站 webhook hook**（评估 ④ 架构级缺口，Wave 3，4 PR）：平台在 agent 生命周期关键点签名 POST 租户注册 URL（webhook 回调式，非任意代码）。**方向已拍板（2026-06-13，对比讨论后，research `2026-06-13-tenant-hook-extension-patterns.md`）**：① 注册面=平台 API CRUD（照 triggers J.10，运行时可改不弹 agent 版本，manifest `hooks` 字段转 deprecated）② 起步事件集=三类（run 终态现成 + 审批/artifact 补 2 个 run_event emit 帧走单事件主轴）③ 投递=自建薄版（复用 MemoryDLQWorker/FeedbackConsumerWorker 形态 + PG 队列表，不引 Svix）④ 仅出站异步（同步改写回调违 fail-open 归 M1-F）。横切公理：fail-open（投递故障不阻 run）/ per-tenant 队列 + per-endpoint 断路器（爆炸半径由跨租户决定）/ 不带平台凭证（HMAC-SHA256 签名）/ SSRF 双段校验。设计 STREAM-HX §13。
  ...
  - [ ] **HX-9 PR3a**：投递引擎 `WebhookDeliveryWorker` 消费 webhook_delivery（list_ready → SSRF 复检 + HMAC-SHA256 签名 POST → 2xx delivered/4xx dead_letter/5xx 退避重试 60s~6h/max5/DLQ + per-endpoint 断路器 + per-tenant 并发上限）+ lifespan 接线 + 计量进 token_usage 面 + 可观测（成功率/DLQ 深度/断路器状态）。自足于 webhook_delivery store。
  - [ ] **HX-9 PR3b（收尾）**：入队扫描——3 源表（agent_run 终态/agent_approval/artifact）加跨租户 list-since 方法 + worker 入队阶段（扫新行 → 匹配 enabled endpoint 的租户+event_type+agent 范围 → 幂等入队 webhook_delivery）→ 端到端闭环。幂等去重靠 UNIQUE(endpoint_id,event_id) 故 since 方法仅扫描量优化。（PR3 拆分 2026-06-13，见 STREAM-HX §13.7）
