...
- **CM-7（B7）结构化 note** P2：摘要"背景非指令"语义+增量更新 ① + writeback 显式操作 ②（A-MEM/Mem0）
  - [ ] **CM-7 PR1 compressor 摘要强化（①）**：preamble + 三段结构 prompt + UPDATE/FRESH 双模式 + `summary.update_mode` 日志 + unit（现有压缩测试不回归）
  - [ ] **CM-7 PR2 writeback 显式操作（②，收尾）**：`flush_messages_to_memory(reconcile=)` + `_reconcile_candidates` + `MemoryPolicy.reconcile_writes` + factory 透传 + `record_memory_reconcile` + tests + 回填
