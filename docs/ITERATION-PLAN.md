...
- **SE-4 重放验证 runner（咽喉）**：拆 SE-4a（判定核心）+ SE-4b（编排核心）+ SE-4c（真 graph adapter）
  ...
  - [ ] **SE-4c 真 graph adapter**：`TaskRunner` 实现——按 candidate 构造两个仅差 `spec.skills` 的 AgentSpec、`build_agent`→`graph.ainvoke` 跑 baseline/treatment、取末条 AI 消息为 answer;高危走 gVisor 沙箱(复用 ExecPythonTool 路径);接真 Haiku judge。integration 真 PG + 真 Haiku 端到端验证
