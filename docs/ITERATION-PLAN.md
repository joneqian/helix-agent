...
- [~] **J.10 调度 / 触发** — cron + webhook 两类 trigger + scheduler 单副本（轮询 `agent_trigger` 表）+ `trigger_run` 表；`event` 触发器 + 分布式 scheduler 推 M1+。**2026-05-20 修订（Mini-ADR J-26）+ 2026-05-22 deer-flow 对比修订（Mini-ADR J-42）**：M0 含 failed run 重试 / DLQ（K7 模式）+ scheduler quota（Stream C.5）+ 每触发器独立 secret 的 webhook 鉴权；`event` 源（PG NOTIFY）推 M1、APScheduler 弃用改轮询。STREAM-J-DESIGN § 16（重写）+ Mini-ADR J-18 / J-26 / J-42。**2026-05-22 启动（6 PR 链）**：
  - [ ] **设计 PR** — STREAM-J-DESIGN § 16 重写 + Mini-ADR J-26 (3)(4) 修订 + 新增 J-42 + 本行
  - [ ] **J.10-step1 数据层** — migration `0033_agent_trigger`（`agent_trigger` + `trigger_run` 两表，tenant RLS）+ ORM + `TriggerKind`/`TriggerSpec` protocol + `TriggerStore` ABC/InMemory/SQL
  - [ ] **J.10-step2 scheduler + cron** — `croniter` + in-process `TriggerScheduler`（reaper 范式）+ cron 触发复用 `run_agent` 发起链 + control-plane lifespan 接线
  - [ ] **J.10-step3 webhook + CRUD API** — `POST /v1/triggers/{id}/webhook` 入站（per-trigger secret 鉴权 + middleware 豁免）+ Trigger CRUD API
  - [ ] **J.10-step4 DLQ + quota** — failed trigger run DLQ 重试（K7 backoff 1m→5m→30m→2h→6h）+ scheduler quota（新 `QuotaDimension`）+ audit 四态
  - [ ] **J.10-step5 eval + 收尾** — `tools/eval/trigger.py` + baseline regen（`J.10_trigger` PASS）+ 零债 6 条
