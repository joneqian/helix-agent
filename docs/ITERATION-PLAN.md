...
- **CM-6（B4）MMR + 时间衰减** P1：时间衰减进 retrieve() + MMR 在 rerank 后（叠加增益自测留 CM-N5）
  - [x] **CM-6 PR1 纯核心**（本次）：helix-common `rrf_fuse_scored`（返回 `(item, score)` 对；`rrf_fuse` 改薄包装、sorted 稳定性保 tie 序字节不变，现有 10 rrf 测试不改即过）+ `search/decay.py`（`temporal_decay_factor`：`0.5+0.5*2^(-age/half_life)`，floor 0.5/负龄钳 0/单调不增）+ `search/mmr.py`（`mmr_select` greedy λ=0.7 cosine：tie 保调用方序/维度不齐与空 embedding 跳过/零范数得 0 不 NaN）+ 7 decay + 7 mmr unit。316 common 回归绿。不接调用方（pure-core 先行）
