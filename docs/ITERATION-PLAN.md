...
- **SE-4 重放验证 runner（咽喉）**：拆 SE-4a（判定核心）+ SE-4b（重放 harness）
  - [x] **SE-4a grounding 判定核心**（PR #TBD）：纯逻辑模块 `orchestrator/evolution/grounding.py`——`decide_grounding` 给逐 case baseline/treatment 产 verdict(pass/fail/inconclusive)。**配对显著性**(SE-A5)手写无 scipy：二元 assertion→McNemar 精确二项、序数 judge 分→Wilcoxon 符号秩(n≤15 精确枚举/否则正态近似)；pass = 无新失败 ∧ p<α(0.05) ∧ delta≥θ(0.08) ∧ n≥n_min。**关键**：双侧精确 p 在 n=5 下限 0.0625>0.05 → `n_min` 默认 **6**(显著性下限,T≥5 仅稳定性)。**信号层级**(SE-A5b)`SignalTier` T1 硬verifier/T2 校准GenRM+锚点/T3 → auto-promote 资格分流(高危永不自动)。`to_eval_result` 组装 `SkillEvalResult`(id/created_at 注入,保持纯)。23 单测 + 530 mypy 绿。**深挖溯源**：本 PR 把 4 轮外部检索(CTA swap-order judge / 执行式验证 / 失败归因 14.2% 天花板 / 1% 锚点防坍缩)落进设计文档 SE-4 设计依据 + SE-A5b。**细化**：SignalTier 持久化、GenRM 置信/judge-人一致率的真实计算推迟 SE-4b/SE-7
  - [ ] **SE-4b 重放 harness**：真跑 agent graph(baseline vs treatment)+ judge(CI scripted / integration 真 Haiku,swap-order 消位置偏)+ 高危沙箱 + `record_eval_result` 落库；held-out 防泄漏（SE-A6）
