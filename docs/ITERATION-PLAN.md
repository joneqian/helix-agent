...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（回归回滚 + set_status 接线 + 审计/指标）
  ...
  - [ ] **SE-7d 回归回滚**：promote 后监控该 skill 关联 run outcome/feedback,变差→自动 `set_status(ARCHIVED)` + 喂 breaker + 新 AuditAction(ROLLED_BACK);需 promoted-skill run-outcome 反馈源（SE-A11）
