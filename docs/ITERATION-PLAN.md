...
- **SE-6 进化 worker（Layer B 引擎）**：拆 SE-6a（co-evolve 编排核心）+ SE-6b（worker 外壳 + 真实接线 + lifespan）
  - [x] **SE-6a co-evolve 编排核心**（PR #TBD）：`control_plane/skill_evolution.py` `evolve()` 纯逻辑——`distill → loop[replay → 若 fail 则 attribute → content 则 revise 再 replay] → grounded/rejected/exhausted/no_draft`,有界轮 `max_rounds`(SkillGen gen-verify-refine + CoEvoSkills 生成/验证分离)。**只读 `verdict`(protocol EvalVerdict)解耦,不引 orchestrator → 无 import cycle**;execution 错不喂回(`should_feed_back` 门控,防坍缩)、inconclusive→rejected(DRAFT 待人审)。4 seam(distill/replay/attribute/revise)注入,CI fake 全测。`RoundRecord` 历史可观测。8 单测 + ruff/mypy 绿（SE-A9）
  - [ ] **SE-6b worker 外壳 + 接线**：克隆 `CurationWorker`(start/stop/_loop/run_once/_bypass_rls/_tenant_scope),扫 candidate→组装成功/失败轨迹+锚点重放集→接真 aux LLM(distiller/attributor)+ GraphReplayTaskRunner + ReplayRunner + SkillStore 落 DRAFT(evolution_origin='distilled'+溯源)→交治理门；wire app lifespan；integration run_once 全链（SE-A9）
