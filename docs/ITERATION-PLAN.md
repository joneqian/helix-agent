...
- [~] **J.15 有状态 per-user 执行环境** — **主体已落地（已纠正状态）**：docker named volume per user + 热沙盒会话 + 空闲 TTL reaper（default 15min）+ restore = 冷启动挂暖卷；不走 CRIU（held-pipe 限制）。supervisor.py + reaper.py + 迁移 0018/0020 已交付。**2026-05-20 设计 PR 修订（STREAM-J-DESIGN § 9.5 + Mini-ADR J-36 lifecycle）**：剩 4 项 M0 补强 → 2 实施 PR：
  - [ ] **J.15-补强-1** Volume quota enforcement（QuotaEnforcer 预检 + 异步对账）+ Volume lifecycle 三档（soft-delete → archive → hard delete，Mini-ADR J-36）—— 迁移 0026 加 `size_limit_bytes` + `deleted_at` + `archived_object_key` 列；新建 `quota_enforcer.py` + `lifecycle.py`
  - [ ] **J.15-补强-2** Volume backup pipeline（daily rsync 到 ObjectStore + 7 天保留 + restore runbook + DLQ）+ at-rest 加密文档化（宿主机 LUKS / 云磁盘加密依赖落实 P0 #9）—— 新建 `volume-backup-worker` + `tools/persistence/restore_volume.py` + `docs/runbooks/volume-restore.md` + `docs/runbooks/deployment.md` 加加密章节；迁移 0027 加 `volume_backup` + `volume_backup_dlq` 表
