...
- **CM-1（A1）运行时 error-as-guidance** P0：通用工具失败→grounded 恢复 advisory 注入主循环（泛化 L-4 `<mutation-advisory>`；注入须源自真实工具/执行信号）
  - [x] **CM-1 PR1 分类器纯核心**（本次）：`graph_builder/error_classifier.py`（`ToolErrorClass` 8 类 taxonomy + `ClassifiedToolError` frozen dataclass + `classify_tool_error` 纯函数：blocked→`blocked_by_policy`、`ToolNotFoundError`→`unknown_tool`、异常类型(Timeout/Permission/FileNotFound)优先于文本模式 + `_is_retryable` 受 `ToolSpec.idempotent`/`resolved_side_effect` 约束(CM-B5：仅 read_only/idempotent 的 transient 才 retryable) + `render_recovery_advisory` 拼 `<recovery-advisory>` + summary 300 截断）+ 21 unit。**不接图、不动 state、不碰 L-4**（pure-core 先行）。975 orchestrator 回归绿（1 integration error 为本地无 Docker）
  - [ ] **CM-1 PR2 接线 + L-4 收敛**：catch 点调 `classify_tool_error` 聚合进 `tool_failures`；`AgentState` `failed_mutations`→`tool_failures` 泛化；`classify_mutation`→产 `ClassifiedToolError(mutation_not_landed)`；`_build_recovery_advisory` 取代 `_build_mutation_advisory`（`<recovery-advisory>`）+ `helix_cm_tool_error_total`/`helix_cm_recovery_advisory_chars` + 迁移 L-4 测试 + e2e。→ CM-1 完成
