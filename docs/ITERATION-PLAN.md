...
- **SE-7 全自动护栏**：拆 SE-7a（auto-promote 策略）+ SE-7b（速率限制 + 熔断）+ SE-7c（promote 接线 + 审计）+ SE-7d（回归回滚）
  ...
  - **SE-7d 回归回滚**：拆 SE-7d-1（归因采集）+ SE-7d-2（判定器）+ SE-7d-3（monitor 接线）。按**专表 `skill_run_usage`**(§4.4) skill-centric 归因（**不借 trajectory metadata**——查询 skill-centric vs trajectory run-centric，建模错配）；**按 skill_version 判定不连坐**（SE-A11）
    ...
    - [ ] **SE-7d-2 判定器**(纯逻辑 CI 可测)：`decide_rollback(*, window, promote_baseline, config) -> RollbackDecision`——复用 SE-4a `grounding.py` McNemar/Wilcoxon 配对显著性 + 绝对地板阈值；`n < n_min` → 不动(防小样本误杀)
    - [ ] **SE-7d-3 monitor + 动作**：周期扫 ACTIVE distilled version → 聚合 `skill_run_usage` 窗口 → 判定 → `set_status(ARCHIVED)` + `breaker.record(ok=False)`(同 `{tenant}:{agent}` scope) + emit `SKILL_EVOLUTION_ROLLED_BACK`(新 AuditAction) + 落 `skill_eval_result` 回滚证据。真路径 integration 验证
