...
- **CM-2（A2）working memory 滑窗** P0：`agent_node` compressor 前廉价轮次截断（保 ToolCall↔ToolResult 配对）
  - [ ] **CM-2 PR1 滑窗纯核心**：`context/working_window.py`（`TrimResult`+`trim_to_recent_turns` 纯函数+`WorkingWindow` dataclass+token-gate，复用 `compressor.estimate_tokens`）+ unit（token-gate/保首轮/配对完整/leading 冻结/三边界/dropped_turns）。不接图、不动 protocol
  - [ ] **CM-2 PR2 接线**：`WorkingMemoryPolicy`→`PolicySpec.working_memory`；factory 构造 `WorkingWindow` 传 `build_react_graph`；`agent_node` 注入前插 `working_window.apply`；`helix_cm_working_window_trim_total{outcome}`+`helix_cm_working_window_dropped_turns`；集成测（长 history 裁剪+checkpoint 完整+compressor 协同）+ 文档标 done
