...
- **M1-3**(#803 ✅)接 `resolve_extends` 进 build(floor 真生效)+ 平台模板 system_admin CRUD API。
- **M1-4**(#804 ✅)租户 fork API(POST 派生 manifest 带 extends)+ `agent_code` 解析。
- **M1-5**(#805/#806/#807/#808 ✅)外部 `(agent_code,user_id)` 运行 API + `agent_instance` 绑定行 + API-key 传 user_id(+ 审计)+ episodic 记忆按 agent 隔离。
- **M1-6** 前端:平台模板管理(system_admin,M1-6a #809 ✅)+ 租户模板市场 + fork UI(M1-6b ✅)。
