...
export const AgentTemplateConfigForm = forwardRef<AgentTemplateConfigFormHandle, Props>(
  function AgentTemplateConfigForm({ editing, onSaved, onSubmittingChange }, ref) {
    ...
    const submit = useMemo(
      () => async (): Promise<void> => {
        ...
        try {
          ...
          message.error(
            err instanceof ApiError ? `${err.code}: ${err.message}` : t("agent_templates.save_failed"),
          );
        } finally {
        ...
        <Form<MetaFields> form={form} layout="vertical">
          ...
          >
            <Select
              ...
              options={TIERS.map((v) => ({ value: v, label: t(`agent_templates.tier_${v}`) }))}
            />
          </Form.Item>
          <Form.Item
            name="status"
          ...
          >
            <Select
              ...
              options={STATUSES.map((v) => ({ value: v, label: t(`agent_templates.status_${v}`) }))}
            />
          </Form.Item>
          <Form.Item
            name="enabled"
